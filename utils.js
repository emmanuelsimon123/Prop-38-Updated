...
/* =========================================================
   HTML Sanitizer (for lesson-provided HTML)
========================================================= */
function sanitizeHTML(html) {
  const ALLOWED_TAGS = new Set(['b','strong','i','em','u','br','span','sub','sup','code','a','p','ul','ol','li']);
  // Use DOMParser to avoid any potential script execution during parsing
    ...
    for (const child of children) {
      if (child.nodeType === Node.ELEMENT_NODE) {
        ...
        if (!ALLOWED_TAGS.has(tag)) {
          // Recursively process children first, then lift them up in place of this element
          walk(child);
          while (child.firstChild) node.insertBefore(child.firstChild, child);
          node.removeChild(child);
        } else {
          // Strip all attributes except <a href> with safe URLs
        ...
        // Remove comments and other non-text, non-element nodes
        node.removeChild(child);
      }
    }
  }
